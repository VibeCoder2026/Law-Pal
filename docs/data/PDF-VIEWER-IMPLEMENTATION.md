...
The PDF system now uses **on-demand downloads** instead of bundling every PDF in the APK.

- **PDF URLs:** `src/assets/acts-pdf-urls.json`
- **Cache location:** `FileSystem.documentDirectory/pdfs/...`
- **UI behavior:** Download prompt + progress, optional "Open in Browser"
- **Constitution deep-linking:** `ActPdfViewer` accepts `initialPage` so the reader opens on the correct page.
- **Reading progress:** Act PDFs remember the last page and resume automatically.
- **Download resilience:** simple retry/backoff for failed downloads.
- **Page index:** `src/assets/constitution-page-index.json` generated by `tools/parse/build-constitution-page-index.py`
- **Bundling change:** `law_sources/**` removed from `assetBundlePatterns`

If the Constitution PDF source changes, regenerate the page index:
```bash
...
**Note:** The sections below describe the original bundled-PDF setup. The current production flow uses on-demand downloads (see "Update (Jan 2026)").
